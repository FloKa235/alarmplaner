/**
 * InventarSection — Professionelle Inventarverwaltung im Szenario-Detail
 *
 * Gleicher Aufbau wie die große InventarPage:
 * - 14-Spalten-Tabelle (sortierbar, sticky Header)
 * - 6 Filter + aktive Filter-Badges
 * - Summary Stats (4 Cards)
 * - Create/Edit Modal + Delete-Dialog
 * - Zeilen-Klick → Edit-Modal
 *
 * Scope: Zeigt nur Items die mit diesem Szenario verlinkt sind.
 * Bei Create wird das aktuelle Szenario automatisch verlinkt.
 */
import { useState, useMemo, useRef, useEffect } from 'react'
import {
  Package, Plus, Loader2, Search, X, Euro,
...
interface InventarSectionProps {
  scenarioId: string
  districtId: string
  scenarioTitle: string
}

// ─── Konstanten (identisch mit InventarPage) ──────────
...
type SortKey =
  ...
  | 'price_per_unit' | 'totalCost'
  | 'location' | 'coverage'

const PRIORITY_ORDER: Record<string, number> = {
  kritisch: 0, hoch: 1, mittel: 2, niedrig: 3,
}

function SortIcon({ col, sortKey, sortDir }: { col: SortKey; sortKey: SortKey | null; sortDir: 'asc' | 'desc' }) {
  if (sortKey !== col) return <ChevronDown className="ml-1 inline h-3 w-3 text-text-muted/40" />
  return sortDir === 'asc'
...
export default function InventarSection({
  scenarioId, districtId, scenarioTitle,
}: InventarSectionProps) {
  void scenarioTitle // used in empty state text below
  ...
  // All inventory items for the district
  const { data: allDistrictItems, loading, refetch } = useSupabaseQuery<DbInventoryItem>(
    (sb) => sb.from('inventory_items').select('*').eq('district_id', districtId).order('category'),
    [districtId],
  )

  // Scenario links for ALL items (needed for scenario column display + filtering)
  const allItemIds = useMemo(() => allDistrictItems.map(i => i.id), [allDistrictItems])
  const { data: allScenarioLinks, refetch: refetchLinks } = useSupabaseQuery<DbInventoryScenarioLink>(
    (sb) => {
      if (allItemIds.length === 0) return sb.from('inventory_scenario_links').select('*').limit(0)
      return sb.from('inventory_scenario_links').select('*').in('inventory_item_id', allItemIds)
    },
    [allItemIds.join(',')],
  )

  // Items linked to THIS scenario (base set before any user filters)
  const scenarioItemIds = useMemo(() => {
    return new Set(
      allScenarioLinks.filter(l => l.scenario_id === scenarioId).map(l => l.inventory_item_id)
    )
  }, [allScenarioLinks, scenarioId])

  const items = useMemo(
    () => allDistrictItems.filter(i => scenarioItemIds.has(i.id)),
    [allDistrictItems, scenarioItemIds],
  )

  const { data: municipalities } = useSupabaseQuery<DbMunicipality>(
    (sb) => sb.from('municipalities').select('id, name').eq('district_id', districtId).order('name'),
    [districtId],
  )

  const { data: allScenarios } = useSupabaseQuery<Pick<DbScenario, 'id' | 'title' | 'handbook'>>(
    (sb) => sb.from('scenarios').select('id, title, handbook').eq('district_id', districtId).order('title'),
    [districtId],
  )
  ...
  const filtered = useMemo(() => {
    ...
    if (filterSzenario) {
      const linkedIds = new Set(
        allScenarioLinks.filter(l => l.scenario_id === filterSzenario).map(l => l.inventory_item_id),
      )
      result = result.filter(i => linkedIds.has(i.id))
    }
    if (filterStatus) {
      result = result.filter(i => {
    ...
    return result
  }, [items, searchQuery, filterKategorie, filterPriority, filterGemeinde, filterSzenario, filterStatus, allScenarioLinks])
  ...
  const sortedItems = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'kategorie': cmp = (a.kategorie ?? '').localeCompare(b.kategorie ?? '', 'de'); break
        case 'priority': {
          const pa = a.priority ? (PRIORITY_ORDER[a.priority] ?? 99) : 99
          const pb = b.priority ? (PRIORITY_ORDER[b.priority] ?? 99) : 99
          cmp = pa - pb; break
        }
        case 'target_quantity': cmp = a.target_quantity - b.target_quantity; break
        case 'current_quantity': cmp = a.current_quantity - b.current_quantity; break
        case 'price_per_unit': cmp = (a.price_per_unit ?? -1) - (b.price_per_unit ?? -1); break
        case 'totalCost': {
          const ca = a.price_per_unit && a.target_quantity ? a.price_per_unit * a.target_quantity : -1
          const cb = b.price_per_unit && b.target_quantity ? b.price_per_unit * b.target_quantity : -1
          cmp = ca - cb; break
        }
        case 'location': cmp = (a.location ?? '').localeCompare(b.location ?? '', 'de'); break
        case 'coverage':
          cmp = getPercent(a.current_quantity, a.target_quantity) - getPercent(b.current_quantity, b.target_quantity); break
      }
  ...
  const clearAllFilters = () => {
    setSearchQuery(''); setFilterKategorie(null); setFilterPriority(null)
    setFilterGemeinde(null); setFilterSzenario(null); setFilterStatus(null)
  }

  // ─── CRUD ───
  const openCreate = () => {
    setEditId(null)
    setForm({ ...EMPTY_FORM, scenario_ids: [scenarioId] })
    setShowModal(true)
  }

  const openEdit = (item: DbInventoryItem) => {
    setEditId(item.id)
    const itemScenarioIds = allScenarioLinks
    ...
    try {
      ...
      if (editId) {
        const { error } = await supabase
          .from('inventory_items').update(payload).eq('id', editId).eq('district_id', districtId)
        if (error) throw error
      } else {
        const { data, error } = await supabase
          .from('inventory_items').insert({ ...payload, district_id: districtId }).select('id').single()
        if (error) throw error
        itemId = data.id
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const { error } = await supabase
        .from('inventory_items').delete().eq('id', deleteTarget.id).eq('district_id', districtId)
      if (error) throw error
      refetch()
      refetchLinks()
  ...
  // ─── Computed stats (on scenario-scoped items) ───
    ...
    return (
      <div className="rounded-2xl border border-border bg-white p-8 text-center">
        ...
        <p className="mb-4 text-sm text-text-secondary">
          Diesem Szenario sind noch keine Inventar-Artikel zugeordnet. Legen Sie Artikel an oder verknüpfen Sie bestehende Artikel über die Inventar-Verwaltung.
        </p>
        <button
          onClick={openCreate}
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-border bg-white p-4">
          ...
          <p className="mt-0.5 text-xs text-text-muted">{usedKategorien.length} Kategorien</p>
        </div>
        <div className="rounded-xl border border-border bg-white p-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select className="rounded-xl border border-border bg-white px-3 py-2 text-sm text-text-secondary outline-none" value={filterKategorie || ''} onChange={e => setFilterKategorie(e.target.value || null)}>
          <option value="">Alle Kategorien</option>
          {KATEGORIE_OPTIONS.map(k => <option key={k} value={k}>{k}</option>)}
        </select>
        <select className="rounded-xl border border-border bg-white px-3 py-2 text-sm text-text-secondary outline-none" value={filterPriority || ''} onChange={e => setFilterPriority(e.target.value || null)}>
          <option value="">Alle Prioritäten</option>
          {Object.entries(PRIORITY_CONFIG).map(([key, cfg]) => <option key={key} value={key}>{cfg.label}</option>)}
        </select>
        <select className="rounded-xl border border-border bg-white px-3 py-2 text-sm text-text-secondary outline-none" value={filterGemeinde || ''} onChange={e => setFilterGemeinde(e.target.value || null)}>
          <option value="">Alle Gemeinden</option>
          {municipalities.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
        </select>
        <select className="rounded-xl border border-border bg-white px-3 py-2 text-sm text-text-secondary outline-none" value={filterSzenario || ''} onChange={e => setFilterSzenario(e.target.value || null)}>
          <option value="">Alle Szenarien</option>
          {allScenarios.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
        </select>
        <select className="rounded-xl border border-border bg-white px-3 py-2 text-sm text-text-secondary outline-none" value={filterStatus || ''} onChange={e => setFilterStatus(e.target.value || null)}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {searchQuery && (
            <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700">
              Suche: &quot;{searchQuery}&quot;
              <button onClick={() => setSearchQuery('')} className="ml-0.5 text-primary-400 hover:text-primary-600"><X className="h-3 w-3" /></button>
            </span>
          )}
          {filterKategorie && (
            <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700">
              {filterKategorie}
              <button onClick={() => setFilterKategorie(null)} className="ml-0.5 text-primary-400 hover:text-primary-600"><X className="h-3 w-3" /></button>
            </span>
          )}
          {filterPriority && (
            <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700">
              {PRIORITY_CONFIG[filterPriority]?.label}
              <button onClick={() => setFilterPriority(null)} className="ml-0.5 text-primary-400 hover:text-primary-600"><X className="h-3 w-3" /></button>
            </span>
          )}
          {filterGemeinde && (
            <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700">
              {municipalities.find(m => m.id === filterGemeinde)?.name}
              <button onClick={() => setFilterGemeinde(null)} className="ml-0.5 text-primary-400 hover:text-primary-600"><X className="h-3 w-3" /></button>
            </span>
          )}
          {filterSzenario && (
            <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700">
              {allScenarios.find(s => s.id === filterSzenario)?.title}
              <button onClick={() => setFilterSzenario(null)} className="ml-0.5 text-primary-400 hover:text-primary-600"><X className="h-3 w-3" /></button>
            </span>
          )}
          {filterStatus && (
            <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700">
              Status: {filterStatus === 'ok' ? 'OK' : filterStatus === 'niedrig' ? 'Niedrig' : 'Kritisch'}
              <button onClick={() => setFilterStatus(null)} className="ml-0.5 text-primary-400 hover:text-primary-600"><X className="h-3 w-3" /></button>
            </span>
          )}
        ...
        {filtered.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-text-secondary">
              {items.length === 0
                ? 'Noch keine Inventar-Einträge für dieses Szenario.'
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border">
                {sortedItems.map(item => {
                  ...
                  const itemScenarios = allScenarioLinks
                    .filter(l => l.inventory_item_id === item.id)
                    .map(l => allScenarios.find(s => s.id === l.scenario_id))
                    .filter(Boolean) as Pick<DbScenario, 'id' | 'title'>[]
                  const totalCost = item.price_per_unit && item.target_quantity
                    ? item.price_per_unit * item.target_quantity : null
                        ...
                        {itemScenarios.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {itemScenarios.slice(0, 2).map(sc => (
                              <span key={sc.id} className="inline-block rounded-full bg-primary-50 px-2 py-0.5 text-[10px] font-medium text-primary-700">
      ...
      <Modal
        open={showModal}
        onClose={() => { setShowModal(false); setShowSzenarioDropdown(false) }}
      ...
      >
        ...
        <div className="mb-4 grid grid-cols-2 gap-4">
          <FormField label="Kategorie">
            <select className={selectClass} value={form.kategorie} onChange={e => setForm({ ...form, kategorie: e.target.value })}>
        ...
        <div className="mb-4 grid grid-cols-2 gap-4">
          <FormField label="Soll-Menge">
            <input type="number" className={inputClass} value={form.target_quantity} onChange={e => setForm({ ...form, target_quantity: e.target.value })} placeholder="z.B. 5000" />
          </FormField>
          <FormField label="Ist-Menge">
            <input type="number" className={inputClass} value={form.current_quantity} onChange={e => setForm({ ...form, current_quantity: e.target.value })} placeholder="z.B. 3200" />
          </FormField>
        </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-4">
          ...
          <FormField label="Preis pro Einheit (€)">
            <input type="number" step="0.01" className={inputClass} value={form.price_per_unit} onChange={e => setForm({ ...form, price_per_unit: e.target.value })} placeholder="z.B. 2.50" />
          </FormField>
        </div>

        <div className="mb-4 grid grid-cols-2 gap-4">
          <FormField label="Lagerort">
            <input className={inputClass} value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} placeholder="z.B. Katastrophenschutzlager Thale" />
          </FormField>
          <FormField label="Gemeinde">
            <select className={selectClass} value={form.municipality_id} onChange={e => setForm({ ...form, municipality_id: e.target.value })}>
        ...
        <div className="mb-4 grid grid-cols-2 gap-4">
          ...
          <FormField label="Verantwortlich">
            <input className={inputClass} value={form.responsible} onChange={e => setForm({ ...form, responsible: e.target.value })} placeholder="z.B. Max Müller, THW" />
          </FormField>
        </div>
          ...
          <div ref={szenarioDropdownRef} className="relative">
            ...
            >
              ...
              {form.scenario_ids.map(sid => {
                ...
                return sc ? (
                  <span key={sid} className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2 py-0.5 text-xs font-medium text-primary-700">
                    {sc.title.length > 25 ? sc.title.slice(0, 23) + '…' : sc.title}
                    <button onClick={e => { e.stopPropagation(); toggleScenario(sid) }} className="text-primary-400 hover:text-primary-600">
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ) : null
            ...
            {showSzenarioDropdown && (
              <div className="absolute z-50 mt-1 max-h-48 w-full overflow-y-auto rounded-xl border border-border bg-white shadow-lg">
                {allScenarios.length === 0 ? (
                  ...
                  allScenarios.map(sc => (
                    <label key={sc.id} className="flex cursor-pointer items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-surface-secondary">
                      <input
                        type="checkbox"
        ...
        <ModalFooter
          onCancel={() => { setShowModal(false); setShowSzenarioDropdown(false) }}
          onSubmit={handleSave}
