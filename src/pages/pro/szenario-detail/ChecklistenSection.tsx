/**
 * ChecklistenSection — Vorbereitungs-Checklisten (ExTrass)
 *
 * 18 Kategorien basierend auf dem ExTrass-Leitfaden (Johanniter/BBK).
 * 4-Status-System: Erfüllt | Teilweise erfüllt | Nicht erfüllt | Kein Bedarf
 * Status-Auswahl per 4-Button-Gruppe rechts neben dem Item-Text.
 * Items sind löschbar, editierbar und neue Items können hinzugefügt werden.
 */
...
export default function ChecklistenSection({
  ...
  // ─── Data Fetching ──────────────────────────────────
  const { data: checklistsRaw, loading, refetch } = useSupabaseQuery<DbChecklist>(
  ...
  // Lokaler State für Optimistic Updates (kein Flackern beim Abhaken)
  const [localData, setLocalData] = useState<DbChecklist[]>([])
  useEffect(() => { setLocalData(checklistsRaw) }, [checklistsRaw])

  // Numerisch sortieren (1, 2, 3, ... 17, 18 statt alphabetisch)
  ...
  // ─── Helper: Lokalen State für eine Checkliste updaten ─
  ...
  // ─── Status setzen (direkt, nicht Toggle) ─────────
  const handleSetStatus = async (checklist: DbChecklist, itemId: string, newStatus: StatusKey) => {
    ...
    try {
      ...
      refetch() // Fehler-Fallback: Server-State wiederherstellen
    }
  }
  ...
  const handleEditItem = async (checklist: DbChecklist, itemId: string) => {
    ...
    // Optimistic: sofort lokal updaten + Edit-Modus beenden
    updateLocalChecklist(checklist.id, updatedItems)
    setEditingItem(null)
    setEditText('')
  ...
  const handleAddItem = async (checklist: DbChecklist) => {
    ...
    // Nächste ID generieren: höchste bestehende Nummer + 1
    const prefix = checklist.title.match(/^(\d+)\./)?.[1] || '0'
    const existingNums = checklist.items
      .map(i => {
    ...
    // Optimistic: sofort lokal hinzufügen + Input zurücksetzen
    updateLocalChecklist(checklist.id, updatedItems)
    setNewItemText('')
    ...
    return (
      <div className="rounded-2xl border border-border bg-white p-8 text-center">
        ...
        <p className="mb-4 text-sm text-text-secondary">
          Erstellen Sie die 18 ExTrass-Checklisten (Johanniter/BBK) zur systematischen Vorbereitung auf dieses Krisenszenario.
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Accordion: 18 Kategorien — numerisch sortiert */}
                  ...
                  {items.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Aktionen rechts: Edit + Delete (on hover) + 4 Status-Buttons */}
                        {!isEditing && (
                          <div className="flex flex-shrink-0 items-center gap-1">
                            {/* Edit + Delete — nur bei Hover sichtbar */}
                            <button
                              onClick={() => { setEditingItem({ checklistId: checklist.id, itemId: item.id }); setEditText(item.text) }}
                              
                              title="Bearbeiten"
                              className="rounded p-1 text-gray-300 opacity-0 transition-all hover:bg-gray-100 hover:text-gray-600 group-hover:opacity-100 disabled:opacity-50"
                            >
      ...
      <p className="text-center text-xs text-text-muted">
        Basierend auf dem ExTrass-Leitfaden (Johanniter/BBK) — individuell anpassbar pro Szenario
      </p>
    </div>
  )
}
