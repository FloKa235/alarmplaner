...
const DEFAULT_TEMPLATES: { title: string; category: Category; description: string; items: string[] }[] = [
  {
    title: 'Krisenstab einberufen',
    category: 'krisenstab',
    description: 'Sofortige Alarmierung und Zusammenkunft des Krisenstabs.',
    items: [
      'Krisenstabsleiter informieren',
      'Alle Krisenstabsmitglieder alarmieren',
      'Lageraum vorbereiten / öffnen',
      'Kommunikationsmittel prüfen (Funk, Telefon, IT)',
      'Lagebild erstellen (erste Meldung)',
      'Einsatztagebuch starten',
      'Zuständigkeiten verteilen (S1–S6)',
      'Erste Lagebesprechung durchführen',
    ],
  },
  {
    title: 'Sofortmaßnahmen',
    category: 'sofortmassnahmen',
    description: 'Erste operative Maßnahmen nach Kriseneintritt.',
    items: [
      'Betroffenes Gebiet absperren / sichern',
      'Rettungskräfte anfordern / disponieren',
      'Evakuierungsbedarf prüfen',
      'Notunterkünfte aktivieren',
      'Bevölkerung warnen (NINA, Sirenen, Durchsagen)',
      'Versorgung sicherstellen (Wasser, Strom, Lebensmittel)',
      'Einsatzkräfte-Logistik koordinieren',
      'Nachbarkreise / Regierungspräsidium informieren',
    ],
  },
  {
    title: 'Krisenkommunikation',
    category: 'kommunikation',
    description: 'Interne und externe Kommunikationsmaßnahmen.',
    items: [
      'Pressesprecher benennen',
      'Erste Pressemeldung verfassen',
      'Bürgertelefon / Hotline einrichten',
      'Social-Media-Kanäle monitoren + bespielen',
      'Interne Mitarbeiter-Info versenden',
      'Regelmäßige Lage-Updates planen',
      'Desinformationen identifizieren + richtigstellen',
    ],
  },
  {
    title: 'Nachbereitung',
    category: 'nachbereitung',
    description: 'Maßnahmen nach Beendigung der akuten Krisenlage.',
    items: [
      'Abschlussbesprechung Krisenstab durchführen',
      'Einsatztagebuch abschließen',
      'Schadenserhebung durchführen',
      'Lessons Learned dokumentieren',
      'Verbesserungsvorschläge sammeln',
      'Dankes-Kommunikation an Einsatzkräfte',
      'Psychosoziale Nachsorge anbieten',
...
export default function ChecklistenProPage() {
  ...
  const createFromTemplate = async (template: typeof DEFAULT_TEMPLATES[0]) => {
    if (!districtId) return
    setSaving(true)
    try {
      const items: ChecklistItem[] = template.items.map((text) => ({
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const { error } = await supabase.from('checklists').delete().eq('id', deleteTarget.id).eq('district_id', districtId)
      if (error) throw error
      refetch()
  ...
  return (
    <div>
      <PageHeader
        title="Krisenstabs-Checklisten"
        description="Operative Checklisten für den Krisenstab – vordefiniert oder individuell."
        badge="Verwaltung"
        actions={
          <button
      ...
      {/* Templates (if no checklists yet) */}
      {checklists.length === 0 && (
        <div className="mb-8">
          <h2 className="mb-4 text-lg font-bold text-text-primary">Vorlagen verwenden</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            {DEFAULT_TEMPLATES.map((tpl) => {
              const cat = categoryConfig[tpl.category]
              return (
                <div key={tpl.title} className="rounded-2xl border border-border bg-white p-6">
                  <div className="mb-2 flex items-center gap-2">
                    <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${cat.bg} ${cat.color}`}>
                      <ClipboardList className="h-4 w-4" />
                    </div>
                    <h3 className="font-bold text-text-primary">{tpl.title}</h3>
                  </div>
                  <p className="mb-3 text-sm text-text-secondary">{tpl.description}</p>
                  <p className="mb-3 text-xs text-text-muted">{tpl.items.length} Aufgaben</p>
                  <button
      ...
      {checklists.length === 0 ? (
        <div className="rounded-2xl border border-border bg-white p-12 text-center">
          ...
          <p className="text-sm text-text-secondary">Noch keine Checklisten vorhanden. Verwenden Sie eine Vorlage oder erstellen Sie eine eigene.</p>
        </div>
      ) : (
      ...
      {/* Templates Button (wenn bereits Checklisten existieren) */}
      {checklists.length > 0 && (
        <div className="mt-6 rounded-2xl border border-dashed border-border bg-white p-6">
          <h2 className="mb-3 text-lg font-bold text-text-primary">Vorlagen</h2>
          <div className="flex flex-wrap gap-2">
            {DEFAULT_TEMPLATES.map((tpl) => {
              const exists = checklists.some((cl) => cl.title === tpl.title)
              return (
                <button
                  key={tpl.title}
                  onClick={() => createFromTemplate(tpl)}
                  ...
                  className={`rounded-xl px-4 py-2 text-sm font-medium transition-colors ${
                    exists
                      ? 'cursor-default bg-surface-secondary text-text-muted'
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Neue Checkliste">
        <FormField label="Titel" required>
          <input
            ...
            placeholder="z.B. Technischer Ausfall – Erstmaßnahmen"
          />
        </FormField>
